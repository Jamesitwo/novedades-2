    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <style dangerouslySetInnerHTML={{__html: `
        @media (max-width: 480px) {
          .tienda-nav { gap: 8px !important; }
          .tienda-nav a { font-size: 12px !important; }
        }
      `}} />
      <nav style={{
        background: 'var(--bg2)', borderBottom: '1px solid var(--border)',
        padding: '12px clamp(12px, 4vw, 24px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(12px)'
      }}>
        <a href="/tienda" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 20 }}>🏆</span>
          <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)' }}>Pizdo</span>
          <span style={{
            background: 'var(--accent)', color: '#fff', fontSize: 10, fontWeight: 700,
            padding: '2px 8px', borderRadius: 10, textTransform: 'uppercase'
          }}>Nuevo</span>
        </a>
        <div className="tienda-nav" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '24px clamp(8px, 4vw, 24px)' }}>
        {children}
      </main>
      <footer style={{
        ...
        <p style={{ margin: 0 }}>© 2026 Pizdo — Todos los productos están sujetos a disponibilidad.</p>
      </footer>
    </div>
  );
}
