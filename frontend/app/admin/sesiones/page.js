        ...
        <div className="table-card">
          <table>
            ...
            <tbody>
              {sesiones.map((sesion) => (
                <tr key={sesion.id}>
                  <td data-label="Usuario" className="td-name">{sesion.usuario.nombre}</td>
                  <td data-label="Rol"><span className={`badge ${sesion.usuario.rol}`}>{sesion.usuario.rol}</span></td>
                  <td data-label="IP" className="td-mono" style={{ fontSize: 12 }}>{sesion.ip || '—'}</td>
                  <td data-label="Navegador" style={{ fontSize: 11, color: 'var(--text2)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {sesion.navegador || '—'}
                  </td>
                  <td data-label="Última actividad" className="td-mono" style={{ fontSize: 12 }}>{formatDate(sesion.ultimaAct)}</td>
                  <td data-label="Expira" className="td-mono" style={{ fontSize: 12 }}>{formatDate(sesion.expiraAt)}</td>
                  <td>
